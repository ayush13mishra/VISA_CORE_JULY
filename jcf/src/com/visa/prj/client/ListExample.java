...
public class ListExample {
	...
	public static void main(String[] args) {
		...
		List<String> names = products.parallelStream().map(p -> p.getName()).collect(Collectors.toList());
		 ...
		 * System.out.println("DDDDD"); return p.getCategory().equals("mobile"); });
		 ...
		 * 
		 * while(iter.hasNext()) { Product p = iter.next(); System.out.println(p);
		 * if(p.getCategory().equals("tv")) { iter.remove(); } }
		 * 
		 * System.out.println("Index based : Avoid"); for(int i = 0; i < products.size()
		 * ; i++) { System.out.println(products.get(i)); }
		 */
	}
